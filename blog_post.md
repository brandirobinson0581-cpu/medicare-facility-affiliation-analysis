...
My logistic-regression model reached 85.6% accuracy and 82.9% recall on providers it had not seen during training. That accuracy is only 1.7 percentage points above the 83.9% majority baseline, so I would not call accuracy alone impressive. The model's 0.948 ranking score means it separates the two groups well across possible cutoffs. However, only 53.3% of the profiles it flags are actually cross-setting, so the false alarms matter.

The largest fitted coefficients involve facility size, primary setting, and affiliation count. More affiliations and a larger maximum connected-facility provider count raise the prediction; a higher average count lowers it. Counts use training providers only. Dialysis and home-health settings raise the score; hospital-centered profiles lower it. These are conditional associations, not causes.

For a realistic planning scenario, I tested a summarized hospital-centered profile with four affiliations while imagining that the setting details had not arrived yet. The model estimated a 56.9% chance that the affiliations cross care settings. I would use that result to prioritize a manual review, not to make an automatic decision. Once the complete facility-type history is available, the team should calculate the answer directly instead of using the model. The data can point us toward a useful question, but a person still needs to verify the answer.
